...
## What is the correlation between the variables in Study 2?

Below is a correlation matrix image for the variables in Study 2. First, I dummy coded whether the SPR box popped up during the query, the language used, the country the IP address was in, and the type of phrase that was searched for (harmful, helpful, and control). 
...
Next, you will notice there is a row with several pink boxes. That row represents when the SPR box to pop up. It means that there is a high correlation between the SPR box popping up in the countries of the United Kingdom, the USA, Ireland, and Australia. There is not a significant correlation with the other countries. We notice the pattern that those are all countries where the primary language is English. 

There is also a high correlation between the SPR box popping up with the harmful phrase searches, rather than control or helpful phrases. Notice the pink box on the bottom right. 
...
## What is the difference between SPR pop ups when a person makes a harmful Google search in various countries and their main language?

Below are two bar plots of the harmful Google queries in Study 2.

As you can see in the plots below, harmful Google queries were routed to the SPR box much more often when they were searched for in English, except in Canada. Harmful Google queries in other languages were much less likely to be routed to the SPR box. 
...
p1 + facet_wrap(~ bHelplineShown) +
    labs(title = "Number of Harmful Google Queries Routed to SPR Box",
       x = "Country Name & Queary Search Language",
       y = "Number of Harmful Queries") + theme(plot.title = element_text(hjust = 0.5))
```
